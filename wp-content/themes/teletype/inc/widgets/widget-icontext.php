...
class Teletype_Icon_Widget extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		$icon =  ( empty( $instance['icon'] ) ) ? '' : '<i class="' . esc_attr( $instance['button_color'] ) . ' icon ' . strip_tags( $instance['icon'] ). '"></i>';
		...
		if ( $url && $button_text ) {
			echo '<p class="text-right"><a href="' . $url . '" class="read-more ' . esc_attr( $instance['button_color'] ) . '">' . $button_text . '</a></p>';
		}
	...
	function form( $instance ) {
		...
		<p><label for="<?php echo $this->get_field_id( 'button_color' ); ?>"><?php _e( 'Icon Color:', 'teletype' ); ?></label>
		<select name="<?php echo $this->get_field_name( 'button_color' ); ?>" id="<?php echo $this->get_field_id( 'button_color' ); ?>" class="widefat">
		<?php
		$options = array(
			'default' => __( 'Default', 'teletype' ),
			'red' => __( 'Red', 'teletype' ),
		);
