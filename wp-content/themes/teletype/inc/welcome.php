...
class Teletype_Welcome {
	...
	public function teletype_welcome_admin_notice() {
		?>
			...
			<p><?php echo sprintf( esc_html__( 'Get a brief setup instructions on the %1$swelcome screen%2$s.', 'teletype' ), '<a href="' . esc_url( admin_url( 'themes.php?page=teletype-about' ) ) . '">', '</a>' ); ?></p>
		...
		<div class="wrap">
			...
			<div class="about-description">
				...
				<?php if ( 'page' == get_option( 'show_on_front' ) && get_option( 'page_on_front' ) ) : ?>
						<p><?php echo sprintf( esc_html__( 'Teletype includes a Front page template. Assign the Front Page template (see Page Attributes box) for the static page selected as Front display, when editing the your %1$sfront page%2$s.', 'teletype' ), '<a href="' . esc_url( get_edit_post_link( get_option( 'page_on_front' ) ) ) . '">', '</a>' ); ?></p>
				<?php endif; ?>
				...
				<?php endif; ?>
						...
						<p><?php _e( 'Need support one-to-one 24/7 by email? Need more options&nbsp;and features? Purchase a Teletype Plus!', 'teletype' ); ?></p>
					...
					<div class="welcome-panel-column">
						...
						<p><?php _e( 'Teletype includes a custom widgets is marked Teletype, go to ', 'teletype' ); ?><a href="<?php echo esc_url( admin_url( 'widgets.php' ) ); ?>"><?php _e( 'Manage widgets', 'teletype' ); ?></a></p>
					...
					<div class="welcome-panel-column welcome-panel-last">
						...
						<h4><?php _e( 'Plugins', 'teletype' ); ?></h4>

<?php if ( !class_exists( 'Projects' ) || !class_exists( 'Woothemes_Features' ) ) { ?>
						<p><?php _e( 'Extend the functionality of the theme using free plugins, go to ', 'teletype' ); ?> <a href="<?php echo esc_url( admin_url( 'themes.php?page=tgmpa-install-plugins' ) ); ?>"><?php _e( 'Install Plugins', 'teletype' ); ?></a></p>
<?php } ?>
					</div>
